...
# -----------------------------------------------------------------------------
# REGION COORDINATES & ALL SUB-REGIONS (전국 17개 시/도 및 모든 시/군/구)
# -----------------------------------------------------------------------------
REGION_COORDS = {
    "서울특별시": {
    ...
    "세종특별자치시": {
        ...
        "sub": [
            ...
            "전동면", "소정면", "한솔동", "도담동", "아름동", "종촌동", "고운동", "보람동", "새롬동"
        ]
    },
    "경기도": {
        ...
        "sub": [
            ...
            "구리시", "군포시", "김포시", "남양주시", "동두천시", "부천시 원미구", "부천시 소사구", "부천시 오정구",
            "성남시 수정구", "성남시 중원구", "성남시 분당구", "수원시 장안구", "수원시 권선구", "수원시 팔달구", "수원시 영통구",
            "시흥시", "안산시 상록구", "안산시 단원구", "안성시", "안양시 만안구", "안양시 동안구", "양주시",
            "양평군", "여주시", "연천군", "오산시", "용인시 처인구", "용인시 기흥구", "용인시 수지구", "의왕시",
    ...
    "충청북도": {
        ...
        "sub": [
            ...
            "진천군", "청주시 상당구", "청주시 서원구", "청주시 흥덕구", "청주시 청원구", "충주시"
        ]
    },
    "충청남도": {
        "lat": 36.5184, "lng": 126.8000,
        "sub": [
    ...
    "전북특별자치도": {
        ...
        "sub": [
            ...
            "익산시", "임실군", "장수군", "전주시 완산구", "전주시 덕진구", "정읍시", "진안군"
        ]
    },
    "전라남도": {
    ...
    "경상북도": {
        "lat": 36.5760, "lng": 128.5056,
        "sub": [
            "경산시", "경주시", "고령군", "구미시", "김천시", "문경시", "봉화군", "상주시",
            "성주군", "영덕군", "영양군", "영주시", "영천시", "울릉군", "울진군", "의성군",
    ...
    "경상남도": {
        ...
        "sub": [
            ...
            "양산시", "의령군", "진주시", "창녕군", "창원시 의창구", "창원시 성산구", "창원시 마산합포구",
            "창원시 마산회원구", "창원시 진해구", "통영시", "하동군", "함안군", "함양군", "합천군"
        ]
    },
    "제주특별자치도": {
...
def load_mock_data():
    ...
    for sido, info in REGION_COORDS.items():
        for sub in info["sub"]:
            # 행정구역별 1~3개의 위험 요인 생성
            for i in range(np.random.randint(1, 4)):
                lat = info["lat"] + np.random.uniform(-0.04, 0.04)
                lng = info["lng"] + np.random.uniform(-0.04, 0.04)
    ...
    for sido, info in REGION_COORDS.items():
        for sub in info["sub"]:
            if np.random.rand() > 0.3: # 약 70% 구역에 사고 이력 생성
...
# 넓은 도(道) 단위와 광역시/단일 구 단위의 줌 레벨 최적화
if selected_sub == "전체":
    map_center = [REGION_COORDS[selected_sido]["lat"], REGION_COORDS[selected_sido]["lng"]]
    if "도" in selected_sido:
        zoom_level = 9  # 넓은 도 단위는 줌아웃
    else:
        zoom_level = 11 # 광역시 단위
else:
